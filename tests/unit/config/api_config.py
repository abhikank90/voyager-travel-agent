...
class FlightAPIConfig(BaseModel):
    ...
    provider: str = Field(default="serpapi", description="API provider: serpapi, skyscanner, etc.")
    ...
    def from_env(cls) -> "FlightAPIConfig":
        return cls(
            ...
            use_mock=not bool(os.getenv("SERPAPI_API_KEY"))
        )


class HotelAPIConfig(BaseModel):
    """Configuration for hotel search APIs."""
    provider: str = Field(default="nuitee", description="API provider: nuitee, expedia, etc.")
    api_key: str | None = Field(default=None)
    base_url: str = Field(default="https://api.liteapi.travel/v3.0")
    booking_url_template: str = Field(
    ...
    def from_env(cls) -> "HotelAPIConfig":
        return cls(
            ...
            use_mock=not bool(os.getenv("NUITEE_API_KEY"))
        )
